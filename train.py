...
osp = OpenSubtitle(config.filename, batch_size=config.seq2seq_batchsize, vocab_size=config.vocab_size, shuffle=True)
data = osp.next()
...
def train(input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, encoder_optimizer,
    ...
    decoder_input = torch.LongTensor([[SOS_TOKEN for _ in range(config.seq2seq_batchsize)]])
    ...
    if use_teacher_forcing:
        ...
        for t in range(max_target_len):
            ...
            decoder_input = torch.LongTensor([[topi[i][0] for i in range(config.seq2seq_batchsize)]])
            decoder_input = decoder_input.to(config.device)
            # Calculate and accumulate loss
            mask_loss, nTotal = maskNLLLoss(decoder_output, target_variable[t], mask[t])
            loss += mask_loss
...
encoder = EncoderRNN(config.hidden_size, embedding, encoder_num_layers, dropout, config.seq2seq_batchsize)
# decoder = LuongAttnDecoderRNN(attn_model, embedding, config.hidden_size, vocab_size, decoder_num_layers, dropout)
decoder = PlainDecoder(attn_model, embedding, config.hidden_size, vocab_size, config.seq2seq_batchsize,
                       decoder_num_layers)
...
# Also change the batch size to deal with single batch size in a chat
forward_encoder = encoder
forward_decoder = decoder
forward_encoder.batch_size = config.chat_batch_size
forward_decoder.batch_size = config.chat_batch_size

# Backward
backward_encoder = EncoderRNN(config.hidden_size, embedding, encoder_num_layers, dropout, config.seq2seq_batchsize)
backward_decoder = PlainDecoder(attn_model, embedding, config.hidden_size, vocab_size, config.seq2seq_batchsize,
                                decoder_num_layers,
                                dropout)
